...
class Bars():
    ...
    def _get_cheapest_bars(self, beer_name = None, partial_match = True):
        ...
        if beer_name == None:
            ...
            (beer_nHHprice_list, beer_nHHprice_index, beer_HHprice_list, beer_HHprice_index, bars_indices) = self._get_beer_infos (beer_name, partial_match)
    ...
    def _filter(self, max_price = 99, min_price = 0, include_HH = True, beer_name = None, partial_match = True):
        ...
        if beer_name is not None:
            ...
            if partial_match is False :
                ...
                bars_indices = np.where(np.char.find(beers_list, beer_name) != -1)
            bars_list = bars_list.iloc[bars_indices[0].tolist()]
...
output1 = test._get_cheapest_bars(beer_name="chouffe")
output = test._filter_price(beer_name="chouffe", partial_match=True)
toc = time()
print(output)
