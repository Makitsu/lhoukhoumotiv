...
with open("../1_mainScrapper/data.csv", mode='r', newline='', encoding='utf-8') as data:
    ...
    while True:
        try:
            ...
            #call of Nomatim to generates GPS coordinates and normalised address from address
            row = next(data)
            address = row['address']
            ...
            if location is None:     #If address not found, save latitude and longitude as TBD
                ...
                with open('longitude_data.csv', mode='a') as file:
                    file.write("TBD"+'\n')
            else:     #Else save it in three separate files
                ...
                with open('location_data.csv', mode='a') as file:
