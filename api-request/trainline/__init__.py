...
class Folders(object):
    ...
    def df(self):
        ...
        for folder in self.folders:
            ...
            summary['destination'].append(None)
            summary['departure_date'].append(folder.departure_date_obj.strftime(_READABLE_DATE_FORMAT))
            summary['arrival_date'].append(folder.arrival_date_obj.strftime(_READABLE_DATE_FORMAT))
            summary['duration'].append(_strfdelta(trip_duration, "{hours:02d}h{minutes:02d}"))
            summary['number_of_segments'].append(folder.segment_nb)
            summary['price'].append(str(folder.price).replace(".", ","))
            summary['currency'].append(folder.currency)
            summary['transportation_mean'].append(folder.transportation_mean)
        df = pd.DataFrame(summary, columns=["departure_date", "arrival_date", "duration", "number_of_segments", "price",
                                            "currency",
                                            "transportation_mean"])
        return df
...
def get_station_id(station_name):
    ...
    global _STATION_DB
    #print("get_station_id")
    if '_STATION_DB' not in globals():
        _STATION_DB = _station_to_dict(_STATIONS_CSV,"station_id")

    station_id = None
    for st_id, st_uic8_sncf in _STATION_DB.items():
        if st_uic8_sncf == station_name:
            station_id = st_id
            break

    if station_id is None:
        KeyError("'{}' station has not been found".format(station_name))
        pass
...
def _get_comfort_class_from_id(comfort_class_obj_list, comfort_class_id):
    ...
    for comfort_class_obj in comfort_class_obj_list:
        #print("_get_comfort_class_from_id")
...
def _filter_folders(folder_list, from_date_obj=None, to_date_obj=None,
                    min_price=0.0, max_price=None, transportation_mean="train",
                    min_segment_nb=1, max_segment_nb=1,
                    bicycle_without_reservation_only=None,
                    bicycle_with_reservation_only=None,
                    bicycle_with_or_without_reservation=None):
...
def _station_to_dict(filename,b, csv_delimiter=';'):
