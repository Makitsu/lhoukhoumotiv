...
_STOP_CITY2 = ['Vannes', 'Auray', 'Lorient', 'Vitré', 'Quimper', 'Quimperlé', 'Rosporden', 'Saint-Malo', 'Dol-de-Bretagne', 'La Roche-sur-Yon', 'Saint-Nazaire', 'La Baule-Escoublac', 'Le Croisic', 'Pornichet', 'Le Pouliguen', 'Sablé-sur-Sarthe', "Les Sables-d'Olonne", 'Biganos', 'Arcachon', 'La Teste-de-Buch', 'Libourne', 'Vendôme', 'Châtellerault', 'Dax', 'Bayonne', 'Biarritz', 'Saint-Jean-de-Luz', 'Hendaye', 'Niort', 'Surgères', 'La Rochelle', "Saint-Maixent-l'École", 'Chasseneuil-du-Poitou', 'Tours', 'Orthez', 'Pau', 'Lourdes', 'Tarbes']
    ...
    for elem in _STOP_CITY:
        if elem not in _STOP_CITY2:
            ...
            if retrieve_general_infos == 'yes':
                ...
                df = pd.DataFrame.from_dict(result)
                print(df)
                df.to_csv('results_wikiscrapping_final_full.csv', sep=";",header=False)
                
scrapping_station_list()

df = pd.read_csv('results_wikiscrapping_final_full.csv',sep=';')
